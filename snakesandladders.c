...
{
int rem;
A:rem=rand()%7;  // To generate dice no's from 1 to 6
if(rem==0)       // if the rem is 0 again we will generate number which is non-zero
goto A;
else
return rem;
}
void displaychart(int curp,char player[4])  //  Function to generate snakes and ladder board with player current position
...
for(i=10;i>0;i--)   // loop to generate even line of board game
{
t=i-1;
if((sft%2)==0)
{
...
for(j=10;j>=1;j--)
{
diceres=(i*j)+(t*c++);    // Formula to generate even line in reverse order
 
if(curp==diceres)        // to check if the number generated matches players current position 
printf("%s\t",player);   // if matches player who is at that position would be printed
else
printf("%d\t",diceres);
...
for(j=1;j<=10;j++)     
{
diceres=(i*j)+(t*c--);  // loop generating odd line of board 
